...
impl ColouredImage {
    fn new(image_array: Vec<Vec<String>>, index: usize) -> Self {
        Self { image_array, index, is_rendered: false }
    }
...
impl Printer{
    pub fn new(image_generator: Converter) -> Self {
        Self{
            image_generator,
    ...
    pub fn get_current_image_array(&self) -> Result<&Vec<Vec<String>>, PrinterError> {
        if !self.has_any_images {
            return Err(PrinterError::NoImagesRegisteredError);
        }
        Ok(self.coloured_images[self.current_image].image_array.as_ref())
    }
    ...
    pub fn move_to_previous_image(&mut self) -> Result<&mut Printer, PrinterError>{
        if !self.has_any_images {
            return Err(PrinterError::NoImagesRegisteredError);
        }
    ...
    pub fn move_to_next_image(&mut self) -> Result<&mut Printer, PrinterError> {
        if !self.has_any_images || self.current_image == self.coloured_images.len() - 1 {
            self.image_generator.next()
                .map_or_else(|| Err(PrinterError::NoImageLeftError), |image_array| Ok(self.add_image(image_array)))
        }else{
            self.current_image += 1;
            Ok(self)
        }
    ...
    fn add_image(&mut self, image_array: Vec<Vec<String>>) -> &mut Printer {
        if !self.has_any_images {
